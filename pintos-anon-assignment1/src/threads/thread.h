...
struct child
  ...
  //可以看到，当未创建子进程时，thread_child初始为自己的tid
    //所以可以用thread_child的store_exit和sema存储thread中st_exit、sema,再用child_elem变相存储在child列表里
  };
...
struct thread
  {
    ...
    /* Structure for Task2 */
    struct list children;                 /* The list of  创建的所有子线程*/
