...
void build_stack_frame (void **esp, int argc, int argv[]);
...
strlcpy(fn_copy,file_name,strlen(file_name)+1);
  ...
  if (success)
    ...
    build_stack_frame (&if_.esp, argc, argv);
    
    //code for testing
    int *argc_ptr = if_.esp+4;  
    printf("argc =\t%d\n", *argc_ptr);

    char ***argv_ptr = (char ***) (if_.esp + 8); 
...
void
build_stack_frame (void **esp, int argc, int argv[]){
...
process_wait (tid_t child_tid UNUSED)
{
  ...
  child_elem = list_begin (l);
  struct child *current_child = NULL;
  while (child_elem != list_end (l))
  {
    /* list_entry:Converts pointer to list element LIST_ELEM into a pointer to
   the structure that LIST_ELEM is embedded inside.  Supply the
   name of the outer structure STRUCT and the member name MEMBER
   of the list element. */
    current_child = list_entry (child_elem, struct child, child_elem);
    if (current_child->tid == child_tid)
    {
      if (!current_child->isrun)
      {
        current_child->isrun = true;
  ...
  if (child_elem == list_end (l)) {
    return -1;
  }
  list_remove (child_elem);
  return current_child->store_exit;
}
